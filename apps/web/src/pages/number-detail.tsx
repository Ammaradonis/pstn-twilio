...
export function NumberDetail() {
  ...
  const outboundCallsQuery = useQuery({
    queryKey: ['calls', numberId, { direction: 'OUTBOUND', limit: 25 }],
    queryFn: () => api.calls.list(numberId!, { direction: 'OUTBOUND', limit: 25 }),
    enabled: Boolean(numberId),
  });

  const recordings = useMemo(() => {
    const items = outboundCallsQuery.data?.items ?? [];
    return items.flatMap((call) => call.recordings.map((recording) => ({ call, recording })));
  }, [outboundCallsQuery.data]);
    ...
    <section className="space-y-6">
      ...
      <div className="mt-6">
        <OutboundVoicemails
          numberId={numberId!}
          items={recordings}
          loading={outboundCallsQuery.isLoading}
        />
      </div>
    </section>
  );
...
function OutboundVoicemails({
  numberId,
  items,
  loading,
}: {
  numberId: string;
  items: Array<{ call: CallDto; recording: CallRecordingDto }>;
  loading: boolean;
}) {
  return (
    <div className="rounded border border-slate-200 bg-white p-4">
      ...
      {!loading && items.length === 0 ? (
        <p className="mt-2 text-sm text-slate-500">No outbound voicemails sent yet.</p>
      ) : null}
